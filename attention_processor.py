...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if proportional_attention:
            attention_scale = math.sqrt(math.log(key.size(2), self.train_seq_len) / head_dim)
        else:
...
class FluxAttnAdaptationProcessor2_0(nn.Module):
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None:
            ...
            # linear proj
            hidden_states = attn.to_out[0](hidden_states) + self.to_out_b(self.to_out_a(hidden_states))
            # dropout
            hidden_states = attn.to_out[1](hidden_states)
            encoder_hidden_states = attn.to_add_out(encoder_hidden_states)
